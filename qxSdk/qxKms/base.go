...
func (m *defaultKmsBaseService) TestMsg(ctx context.Context, params *qxTypesKms.KmsTestMsgReq) (result *qxTypesKms.KmsTestMsgResp, err error) {
	...
	encryptMsg, err := m.qxCtx.Cli.Parser.Encrypt(msg)
	if err != nil {
	...
	res, err := m.qxCtx.Cli.EasyNewRequest(ctx, "/kms/testMsg", http.MethodPost, &qxTypesCcs.CcsTestMsgReq{
		Msg: encryptMsg,
	})
	...
	logx.Infof("打印解析器的状态 %s", m.qxCtx.Cli.Parser.FormatStatus())
	// note: 使用aes解密数据
	decryptMsg, err := m.qxCtx.Cli.Parser.Decrypt(tmpResultData.Data)
	if err != nil {
