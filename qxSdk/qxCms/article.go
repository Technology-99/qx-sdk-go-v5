...
type (
	CmsArticleService interface {
		TestMsg(ctx context.Context, params *qxTypesCms.CmsTestMsgReq) (result *qxTypesCms.CmsTestMsgResp, err error)
	}
	defaultCmsArticleService struct {
		cli *qxCli.QxClient
	}
)

func NewCmsArticleService(cli *qxCli.QxClient) CmsArticleService {
	return &defaultCmsArticleService{
		cli: cli,
	}
}

func (m *defaultCmsArticleService) TestMsg(ctx context.Context, params *qxTypesCms.CmsTestMsgReq) (result *qxTypesCms.CmsTestMsgResp, err error) {
	result = &qxTypesCms.CmsTestMsgResp{}
	reqFn := m.cli.EasyNewRequest(ctx, "/cms/testMsg", http.MethodPost, &params)
	res, err := reqFn()
	if err != nil {
		logx.Errorf("CmsArticleService TestMsg request error: %v", err)
		return nil, err
	}
	logx.Infof("CmsArticleService TestMsg request response: %s", res)
	...
	if m.cli.Config.EncryptionPrivateKey != "" {
		...
		if err != nil {
			logx.Errorf("CmsArticleService TestMsg RsaDecrypt error: %v", err)
			return nil, err
		}
		if err = json.Unmarshal(tmpDataBt, &result.Data); err != nil {
			logx.Errorf("CmsArticleService TestMsg json.Unmarshal error: %v", err)
			return nil, err
		}
	} else {
		logx.Infof("Please contact your system administrator to download and deploy the decryption certificate")
		logx.Infof("CmsArticleService TestMsg response decrypt need encryptionPrivateKey")
	}
